            ...
            <div class="container-fluid" style="margin: 0 240px;">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                            <h1 style="text-align:center " class="card-title"> ادارة المؤلفين </h1>
...
{
    $q=mysqli_query($conn,"SELECT * from authors where AuthorName like '%$_POST[search]%' ORDER BY id_au desc");

    if(mysqli_num_rows($q)==0)
      {
        echo "<h1 style='padding-top: 100px;
        text-align: center;'>عفوا لا يوجد مؤلف بهذا الاسم. حاول البحث مره اخرى</h1>";
      }
            else{ ?>
...
?>
                ...
                <td><a href="manage-author.php?id_au=<?php echo $row['id_au'];?>"><button
                            style="padding: 5px 20px;" class="btn btn-danger">حذف
                        </button></a> </td>
                <td><a href="edit-author.php?id_au=<?php echo $row['id_au'];?>"><button
...
<?php
    $count=0;
    $q=mysqli_query($conn,"SELECT * from authors ORDER BY id_au desc");
    while($row=mysqli_fetch_assoc($q))
...
if(isset($_GET['id_au'])){
    $id=$_GET['id_au'];
$query="DELETE FROM `authors`WHERE id_au= '$id'";
